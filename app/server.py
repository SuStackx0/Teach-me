...
@app.get("/api/review/content/{slug}")
def get_review_content(slug: str):
    """Return cached review content generated by Claude Code on session completion."""
    cached = DB.meta_get(_CONN, f"review_content:{slug}")
    if cached:
        return json.loads(cached)
    lesson = DB.get_lesson_by_slug(_CONN, slug)
    if not lesson:
        raise HTTPException(status_code=404, detail="Lesson not found")
    raise HTTPException(status_code=404, detail="Review content not yet generated")


class ReviewLogPayload(BaseModel):
    slug: str
    rating: int  # 1=struggled, 2=fuzzy, 3=knew_it


@app.post("/api/review/log")
def log_review(payload: ReviewLogPayload):
    row = _CONN.execute("SELECT slug FROM sessions WHERE slug=?", (payload.slug,)).fetchone()
    if not row:
        raise HTTPException(status_code=404, detail="Session not found")

    days = {3: 28, 2: 14, 1: 7}.get(payload.rating, 7)
    today = datetime.now().date()
    new_date = today + timedelta(days=days)

    # Uniqueness guard: don't collide with another session's next_review_date
    existing = {
        r["next_review_date"]
        for r in _CONN.execute(
            "SELECT next_review_date FROM sessions WHERE slug != ? AND next_review_date IS NOT NULL",
            (payload.slug,)
        ).fetchall()
    }
    while new_date.strftime("%Y-%m-%d") in existing:
        new_date += timedelta(days=1)

    new_date_str = new_date.strftime("%Y-%m-%d")
    _CONN.execute(
        "UPDATE sessions SET next_review_date=?, self_rating=? WHERE slug=?",
        (new_date_str, payload.rating, payload.slug)
    )
    _CONN.commit()
    return {"ok": True, "next_review_date": new_date_str, "days_until": (new_date - today).days}
